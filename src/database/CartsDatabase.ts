...
export class CartsDatabase extends BaseDatabase {
    ...
    public async getCartByOwner(ownerId: string): Promise<undefined[] | cartDB[]> {
        ...
        const cartFound = await this.dbConnection(CartsDatabase.TABLE_CARTS)
    ...
    public async addProductToCart(usersCartProductAdded: cartDB) {
        await this.dbConnection(CartsDatabase.TABLE_CARTS).insert(usersCartProductAdded)
    }

    public async updateCart(newCart: cartDB): Promise<void> {

        await this.dbConnection(CartsDatabase.TABLE_CARTS)
            .update({ newCart })
            .where({
