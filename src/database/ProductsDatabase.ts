...
export class ProductsDatabase extends BaseDatabase {
    ...
    public async getProductById(idSearched: string): Promise<productDB | undefined> {
        const [productFound] = await this
            .dbConnection(ProductsDatabase.TABLE_PRODUCTS)
            .where({ id: idSearched })
        return productFound
    }
    public async getProductByName(nameSearched: string): Promise<productDB | undefined> {
        const [productsFound] = await this
            .dbConnection(ProductsDatabase.TABLE_PRODUCTS)
    ...
    public async deleteProductById(idToDelete: string): Promise<void> {
        await this
            .dbConnection(ProductsDatabase.TABLE_PRODUCTS)
            .del()
            .where({ id: idToDelete })
