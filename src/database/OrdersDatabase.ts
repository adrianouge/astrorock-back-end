...
export class OrdersDatabase extends BaseDatabase {
    ...
    public async createNewOrder(newOrder: orderDB): Promise<void> {
        await this.dbConnection(OrdersDatabase.TABLE_ORDERS).insert(newOrder)
    }

    public async getOrderById(orderId: string): Promise<orderDB | undefined> {
        const [orderFound] = await this.dbConnection(OrdersDatabase.TABLE_ORDERS).where({id: orderId})

        return orderFound
    }

    public async getOrdersByUser(userId: string): Promise<orderDB[] | undefined[]> {
        const usersOrders = await this.dbConnection(OrdersDatabase.TABLE_ORDERS).where({userId})
        return usersOrders
    }

    public async updateOrder(orderUpdated: orderDB): Promise<void> {
        await this.dbConnection(OrdersDatabase.TABLE_ORDERS).update(orderUpdated).where({ id: orderUpdated.id })
    }

    public async deleteOrder(orderToDeleteId: string): Promise<void> {
        await this.dbConnection(OrdersDatabase.TABLE_ORDERS).del().where({ id: orderToDeleteId })
    }
}