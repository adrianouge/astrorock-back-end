...
describe("getProductsByNameLike", () => {
    ...
    test("deve retornar mensagem com produtos encontrados com nome parecidos com a pesquisa",
        async () => {
            expect.assertions(2)
            ...
            expect(response.message)
                .toBe(`Produto encontrado:`)
            expect(response.productsFound)
                .toStrictEqual(productExpected)
        }
    )
})
